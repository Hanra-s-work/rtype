    ...
    namespace ECS
    {
        namespace Online
        {
            ...
            class Orchestrator : public EntityNode {
                ...
                /**
                 * @brief Spawns a new play entity in the game.
                 */
                void _spawn(const std::uint32_t id, const std::pair<float, float> &pos);

                /**
                 * @brief Create a bullet for the player that will be controlled by the server.
                 *
                 * @param id The unique identifier of the bullet
                 * @param pos the position the bullet should take
                 */
                void _spawnFriendBullet(const std::uint32_t id, const std::pair<float, float> &pos);

                /**
                 * @brief Create a bullet for the enemy that will be controlled by the server.
                 *
                 * @param id The unique identifier of the bullet
                 * @param pos the position the bullet should take
                 */
                void _spawnEnemyBullet(const std::uint32_t id, const std::pair<float, float> &pos);

                /**
                 * @brief Set the position of an entity based on it's id
                 *
                 * @param id The unique identifier of the entity
                 * @param pos The position to be set
                 */
                void _setPosition(const std::uint32_t id, const std::pair<float, float> &pos);

                /**
                 * @brief Kill an entity based on it's id
                 *
                 * @param id The unique identifier of the entity
                 */
                void _killEntity(const std::uint32_t id);

                /**
                 * @brief Send a message to the server
                 *
                 * @param node The message to be sent.
                 */
                void _sendMessage(const GUI::Network::MessageNode &node);
                 ...
                 */
                void _spawnEnemy(const std::uint32_t id, const std::pair<float, float> pos);
                ...
                std::size_t _playerId = 0;                                                             //!< The unique identifier for the player.
                ...
                std::vector<GUI::ECS::Online::Bullet> _playerBullets;                                  //!< Container for all bullets in the game.
                std::vector<GUI::ECS::Online::Bullet> _enemyBullets;                                   //!< Container for all bullets in the game.
                ...
                std::vector<std::shared_ptr<GUI::ECS::Online::PlayerBrain>> _playerBrain;              //!< Pointer to the player's brain logic.
                std::vector<std::shared_ptr<GUI::ECS::Online::EnemyBrain>> _enemyBrain;                //!< Container for enemy brains.
                std::unordered_map<std::type_index, std::vector<std::any>> _ecsEntities;               //!< ECS entities managed by the orchestrator.
                std::optional<std::shared_ptr<GUI::Network::ThreadCapsule>> _network;                  //!< Container for the network component.
            };
